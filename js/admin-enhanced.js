// Photo Admin Application - Enhanced v2.0
// Features: EXIF extraction, image optimization, batch editing, live preview, secure password handling
...
class PhotoAdminApp {
    ...
    loadConfig() {
        const defaults = {
            ...
            githubToken: '',
            password: '', // Will be loaded from config.json - NEVER store password in localStorage
            maxWidth: 1920,
            jpegQuality: 85,
            autoOptimize: true
        };
    ...
    async loadServerConfig() {
        // Check for environment variables first (Node.js/server environments)
        if (typeof process !== 'undefined' && process.env) {
            if (process.env.GITHUB_TOKEN) this.config.githubToken = process.env.GITHUB_TOKEN;
            if (process.env.REPO_OWNER) this.config.repoOwner = process.env.REPO_OWNER;
            if (process.env.REPO_NAME) this.config.repoName = process.env.REPO_NAME;
            if (process.env.MAX_WIDTH) this.config.maxWidth = parseInt(process.env.MAX_WIDTH);
            if (process.env.JPEG_QUALITY) this.config.jpegQuality = parseInt(process.env.JPEG_QUALITY);
            if (process.env.AUTO_OPTIMIZE) this.config.autoOptimize = process.env.AUTO_OPTIMIZE === 'true';
        }

        // Then try config.json (client-side browser environments)
        try {
            const response = await fetch('config.json');
            if (response.ok) {
                const serverConfig = await response.json();
                
                // Apply server config values (but don't override if already set from env vars or localStorage)
                if (serverConfig.githubToken && !this.config.githubToken) {
                    this.config.githubToken = serverConfig.githubToken;
                }
                if (serverConfig.repoOwner && this.config.repoOwner === 'hansbbans') {
                    this.config.repoOwner = serverConfig.repoOwner;
                }
                if (serverConfig.repoName && this.config.repoName === 'personal-site') {
                    this.config.repoName = serverConfig.repoName;
                }
                if (serverConfig.imageOptimization) {
                    this.config.maxWidth = serverConfig.imageOptimization.maxWidth || this.config.maxWidth;
                    this.config.jpegQuality = serverConfig.imageOptimization.jpegQuality || this.config.jpegQuality;
                    this.config.autoOptimize = serverConfig.imageOptimization.autoOptimize !== undefined 
                        ? serverConfig.imageOptimization.autoOptimize 
                        : this.config.autoOptimize;
                }
            }
        } catch (err) {
            console.log('config.json not available, using defaults or environment variables');
        }
    ...
    updateSettingsForm() {
        ['repoOwner', 'repoName', 'githubToken', 'maxWidth', 'jpegQuality', 'autoOptimize'].forEach(id => {
            ...
            if (el && this.config[id] !== undefined) {
                if (el.type === 'checkbox') {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (!config.adminPassword) {
                    this.showConfigError('adminPassword not found in config.json');
                    throw new Error('adminPassword not configured in config.json');
                }
                return config.adminPassword;
            } else if (response.status === 404) {
                this.showConfigError('config.json not found. Please create it from config.example.json');
                throw new Error('config.json not found');
            }
        } catch (err) {
            if (err.message !== 'config.json not found') {
                console.error('Failed to load config.json:', err);
                this.showConfigError('Failed to load config.json. Check console for details.');
            }
            throw err;
        }
    }
    ...
    showConfigError(message) {
        ...
        const hintDiv = document.querySelector('.password-form .hint');
        ...
        if (hintDiv) {
            hintDiv.innerHTML = `
                ...
                1. Copy <code>config.example.json</code> to <code>config.json</code><br>
                2. Add your admin password to config.json<br>
                3. Refresh this page
            `;
    ...
    saveConfig() {
        localStorage.setItem('photoAdminConfig', JSON.stringify(this.config));
    ...
    renderPhotoGrid() {
        ...
        grid.innerHTML = visiblePhotos.map(photo => {
            const hasChanges = this.changes.has(photo.id);
            const isDeleted = this.deletedPhotos.has(photo.id);
            ...
            <div class="photo-card ${this.selectedPhotos.has(photo.id) ? 'selected' : ''}" data-photo-id="${photo.id}">
                ...
                <div class="photo-details">
                    ...
                    ${hasChanges ? '<div style="color: #f59e0b; font-size: 0.85rem; margin-top: 0.5rem;">● Unsaved changes</div>' : ''}
                </div>
            </div>
            `;
        }).join('');
                    ...
                    img.onload = () => {
                        EXIF.getData(img, function() {
                            ...
                            if (exifData.gpsLat && exifData.gpsLng) {
                                const lat = exifData.gpsLat;
                                const lng = exifData.gpsLng;
                                exifData.location = `GPS: ${lat}, ${lng}`;
                            }
    ...
    renderUploadPreview() {
        ...
        const savingsPercent = totalOptimized < totalOriginal ? (savings / totalOriginal * 100).toFixed(1) : 0;
    ...
    async optimizeImage(upload) {
        if (!this.pica) {
            console.log('Pica not available, skipping optimization');
            return;
        }
    ...
    async addPhotosToHtml(uploads) {
        ...
        if (markerPos === -1) {
            throw new Error('Could not find photos-grid in photos.html');
        }
        
        html = html.substring(0, markerPos + marker.length) + '\n' + newPhotoBlocks + html.substring(markerPos + marker.length);
    ...
    generatePreviewHtml() {
        ...
        const gridEnd = html.lastIndexOf('</div>');
        
        // Find the closing div for photos-grid
        let depth = 1;
        let pos = gridStart;
        let actualGridEnd = gridStart;
        ...
        while (depth > 0 && pos < html.length) {
            ...
            if (nextOpen !== -1 && nextOpen < nextClose) {
                ...
                pos = nextClose + 6;
                if (depth === 0) {
                    actualGridEnd = nextClose;
                }
            }
        }
        
        html = html.substring(0, gridStart) + '\n' + photoBlocks + '\n                ' + html.substring(actualGridEnd);
        ...
        try {
            ...
            const response = await fetch(
                `https://api.github.com/repos/${this.config.repoOwner}/${this.config.repoName}/contents/photos.html`,
                {
                    ...
                    body: JSON.stringify({
                        message: `Update ${this.changes.size} photo(s), delete ${this.deletedPhotos.size} photo(s)`,
    ...
    filterPhotos() {
        ...
        this.filteredPhotos = this.photos.filter(photo => {
            const matchesSearch = !search || 
                ...
                photo.alt.toLowerCase().includes(search) ||
...
// Global wrapper functions for onclick handlers (backward compatibility)
function checkPassword() { return app?.checkPassword(); }
function logout() { return app?.logout(); }
function showSettings() { return app?.showSettings(); }
function closeSettings() { return app?.closeSettings(); }
function saveSettings() { return app?.saveSettings(); }
function switchTab(tab) { return app?.switchTab(tab); }
function filterPhotos() { return app?.filterPhotos(); }
function loadExistingPhotos() { return app?.loadExistingPhotos(); }
function clearUpload() { return app?.clearUpload(); }
function processUpload() { return app?.processUpload(); }
function selectAll() { return app?.selectAll(); }
function deselectAll() { return app?.deselectAll(); }
function selectByYear() { return app?.selectByYear(); }
function selectByLocation() { return app?.selectByLocation(); }
function applyBatchEdit() { return app?.applyBatchEdit(); }
function cancelBatchEdit() { return app?.cancelBatchEdit(); }
function showPreview() { return app?.showPreview(); }
function closePreview() { return app?.closePreview(); }
function commitChanges() { return app?.commitChanges(); }
function hideStatus() { return app?.hideStatus(); }
function closeEditModal() { return app?.closeEditModal(); }
function savePhotoEdit() { return app?.savePhotoEdit(); }
